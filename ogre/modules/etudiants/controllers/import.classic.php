...
class importCtrl extends jController {
    ...
    */
    ...
    function doimport_pv(){
        ...
        $form = jForms::fill('import_pv');
        
        $realname = $_FILES['csv_pv']['name'];
        
        $name = date('dMY') . '-' . time() . '.csv' ;
        
        if( !$form->saveFile('csv_pv', JELIX_APP_VAR_PATH.'uploads/csv_pv/', $name)) {
            jMessage::add("L'importation a echouée...");
            $rep->action = 'etudiants~import:index2';
            return $rep;
        }

        jForms::destroy('import_pv');


        jClasses::inc('utils~NotePv');
        

        $csvParser = new NotePv(JELIX_APP_VAR_PATH.'uploads/csv_pv/'.$name);
        ...
        Logger::log('import_pv', $realname,999);
